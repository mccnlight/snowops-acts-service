...
func New(cfg *config.Config, log zerolog.Logger) (*gorm.DB, error) {
	...
	database, err := gorm.Open(postgres.Open(dbCfg.DSN), &gorm.Config{Logger: gormLogger})
	if err != nil {
		return nil, err
	}
	...
	if dbCfg.ConnMaxLifetime != "" {
		if d, err := time.ParseDuration(dbCfg.ConnMaxLifetime); err == nil {
...
func HealthCheck(ctx context.Context, database *gorm.DB) error {
	return database.WithContext(ctx).Exec("SELECT 1").Error
}
